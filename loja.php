...
<body>
    ...
    <a href="formularioProdutos.html" id="adicionar">Adicionar Produto</a>
    <br><br>
    <label for="produto">Produtos Cadastrados:</label>
    <select id="produto" onchange="mostrarInformacoes()">
        <option value="">Selecione um produto</option>
        <?php foreach ($produtos as $produto): ?>
            <option value="<?= $produto['id'] ?>" data-preco="<?= $produto['valor'] ?>" data-validade="<?= $produto['validade'] ?>" data-estoque="<?= $produto['quantidade'] ?>"><?= htmlspecialchars($produto['nome']) ?></option>
        <?php endforeach; ?>
    </select>
    <div id="informacoes" style="display: none;">
        ...
        <form method="POST" action="carrinho.php">
            ...
            <button type="submit" id="adicionar_carrinho">Adicionar ao Carrinho</button>
            <a href="carrinho.php" id="ver_carrinho">Ver Carrinho</a>
        </form>
        <br>
